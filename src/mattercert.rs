use p256::NistP256;
use x509_cert::{certificate::CertificateInner, der::{Decode, DecodePem}};
use anyhow::{Result, Context};

use crate::{cryptoutil, tlv::{self, TlvBuffer}};
...
fn extract_extension(cert: &x509_cert::TbsCertificate, oid: &str) -> Result<Vec<u8>> {
    let extensions = cert.extensions.as_ref().context("can't get cert extensions")?;
...
fn convert_x509_to_matter_int(cert: &CertificateInner) -> Result<Vec<u8>> {
    ...
    enc.write_uint32(4, (not_before.to_unix_duration().as_secs()-946684800) as u32)?;
    let not_after = cert.tbs_certificate.validity.not_after;
    enc.write_uint32(5, (not_after.to_unix_duration().as_secs()-946684800) as u32)?;
    ...
    let subject_public_key = cert.tbs_certificate.subject_public_key_info.subject_public_key.as_bytes().context("can't extract subject public key")?;
    ...
    let sig = cert.signature.as_bytes().context("can't get signature from x509")?;
