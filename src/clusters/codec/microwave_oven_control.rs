//! Generated Matter TLV encoders and decoders for Microwave Oven Control Cluster
//! Cluster ID: 0x005F
//! 
...
pub fn encode_set_cooking_parameters(cook_mode: u8, cook_time: u8, power_setting: u8, watt_setting_index: u8, start_after_setting: bool) -> anyhow::Result<Vec<u8>> {
    let tlv = tlv::TlvItemEnc {
        tag: 0,
        value: tlv::TlvItemValueEnc::StructInvisible(vec![
        (0, tlv::TlvItemValueEnc::UInt8(cook_mode)).into(),
        (1, tlv::TlvItemValueEnc::UInt8(cook_time)).into(),
...
pub fn encode_add_more_time(time_to_add: u8) -> anyhow::Result<Vec<u8>> {
    let tlv = tlv::TlvItemEnc {
        tag: 0,
        value: tlv::TlvItemValueEnc::StructInvisible(vec![
        (0, tlv::TlvItemValueEnc::UInt8(time_to_add)).into(),
        ]),
    };
    Ok(tlv.encode()?)
}
...
pub fn decode_cook_time(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
...
pub fn decode_max_cook_time(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
    if let tlv::TlvItemValue::Int(v) = inp {
        Ok(*v as u8)
    } else {
        Err(anyhow::anyhow!("Expected Integer"))
