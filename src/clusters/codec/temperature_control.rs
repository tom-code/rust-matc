//! Generated Matter TLV encoders and decoders for Temperature Control Cluster
//! Cluster ID: 0x0056
//! 
...
pub fn encode_set_temperature(target_temperature: u8, target_temperature_level: u8) -> anyhow::Result<Vec<u8>> {
    let tlv = tlv::TlvItemEnc {
        tag: 0,
        value: tlv::TlvItemValueEnc::StructInvisible(vec![
        (0, tlv::TlvItemValueEnc::UInt8(target_temperature)).into(),
...
pub fn decode_temperature_setpoint(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
...
pub fn decode_min_temperature(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
    if let tlv::TlvItemValue::Int(v) = inp {
        Ok(*v as u8)
    } else {
        Err(anyhow::anyhow!("Expected Integer"))
...
pub fn decode_max_temperature(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
    if let tlv::TlvItemValue::Int(v) = inp {
        Ok(*v as u8)
    } else {
        Err(anyhow::anyhow!("Expected Integer"))
...
pub fn decode_step(inp: &tlv::TlvItemValue) -> anyhow::Result<u8> {
    if let tlv::TlvItemValue::Int(v) = inp {
        Ok(*v as u8)
    } else {
        Err(anyhow::anyhow!("Expected Integer"))
